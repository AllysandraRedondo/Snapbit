...
def apply_mustache_filter(frame, landmarks, filter_img):
    h, w, _ = frame.shape
    
    # Landmarks for scaling and orientation
    ...
    upper_lip_center_x, upper_lip_center_y = int(upper_lip_center.x * w), int(upper_lip_center.y * h)

    # Scaling
    face_width = abs(right_x - left_x)
    filter_w = int(1.0 * face_width)
    ...
    # Positioning
    anchor_x = upper_lip_center_x
    vertical_offset = int(filter_h * 0.4) 
    anchor_y = upper_lip_center_y - vertical_offset
